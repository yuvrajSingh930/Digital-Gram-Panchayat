...
const DivorceCertificate_Comp = () => {
  ...
  const fetchForms = useCallback(async () => {
    ...
    if (isUser) {
      ...
      q = collection(firestore, "divorceCertificates");
    }
  ...
  const handleCreateOrUpdate = async (data) => {
    try {
      ...
      if (selectedForm) {
        await updateDoc(
          doc(firestore, "divorceCertificates", selectedForm.id),
          defaultData,
          formData
        );
        alert("Form updated successfully!");
      } else {
        await addDoc(collection(firestore, "divorceCertificates"), formData);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(firestore, "divorceCertificates", id));
  ...
  const handleStatusChange = async (id, status) => {
    try {
      await updateDoc(doc(firestore, "divorceCertificates", id), { status });
  ...
  return (
    ...
    <div className="container">
      <h1 className="title">(Example Add) Divorce Certificate Application</h1>
...
export default DivorceCertificate_Comp;
// _____  _  __
// / ____|| |/ /
// | (___  | ' /
//  \___ \ |  <
